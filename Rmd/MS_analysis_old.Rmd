...
ms <- ms %>% dplyr::mutate(ID = 1:nrow(ms)) %>% separate_rows(ProteinName, sep = ";") %>%
  dplyr::filter(!grepl("^tr", ProteinName))
```
...
## MQ results gene ID conversion

We need to match proteins that were detected by MS to the genes identified by RNA-seq. For this, we use the org.Mm.eg.db R package. 
...
Now we can add the UniprotIDs to the RNA-seq results and the Ensembl IDs to the Mass spec results.
```{r}
rnaseq <- rnaseq %>% left_join(map_tab, by = c("gene_id" = "EnsemblID"))
ms <- ms %>% left_join(map_tab)

## Adding the RNAseq results to the MS results (discarding genes that were not detected by MS)
dat <- ms %>% left_join(rnaseq, suffix = c("ms", "RNAseq"))

##rename the MS logFC and RNAseq logFC
dat <- dat %>% dplyr::rename(log2FC_MS = log2FC)
dat <- dat %>% dplyr::rename(log2FC_RNAseq = logFC)

## label significantly up or down regulated genes
dat$RNAseq_DE <- "not significant"
dat$RNAseq_DE[dat$FDR < 0.05 & dat$log2FC_RNAseq > 0] <- "up"
dat$RNAseq_DE[dat$FDR < 0.05 & dat$log2FC_RNAseq < 0] <- "down"
```
...
```{r}
fus_targets <- fus_targets %>% full_join(map_tab, by = c("gene_id" = "EnsemblID"))
dat <- dat %>% left_join(fus_targets %>% 
                           dplyr::select(nr_peaks, gene_cluster, UniprotID))

dat$FUS_target <- ifelse(!is.na(dat$nr_peaks), "Yes", "No")
```
...
```{r}
write.table(dat, file = here("mass_spec/MQ_report_RNAseq_CLIPtargets.txt"), 
...
```{r}
write.table(dat %>% dplyr::filter(FUS_target == "Yes") %>%
...
```{r}
dat1 <- dat %>% dplyr::filter(!is.na(log2FC_RNAseq))
nrow(dat) - nrow(dat1)
nrow(dat1)
...
```{r}
dat_ms <- dat %>% dplyr::select(P.Value, adj.P.Val, log2FC_MS, gene_name, UniprotName) %>% 
dplyr::rename(PValue = P.Value, adjPValue = adj.P.Val, logFC = log2FC_MS, 
              gene_name = gene_name, protein_name = UniprotName) %>%
...
```{r}
changed <- dat %>% dplyr::filter(adj.P.Val <= 0.05 | log2FC_MS < -1 | log2FC_MS > 1) %>%
...
dup <- !duplicated(dat$EntrezID) %>% head
m <- match(dat[dup]$gene_id, genes$gene_id)
...
goana_changed <- goana(de = unique(changed$EntrezID), species = "Mm", 
                   universe = dat[dup]$EntrezID, covariate = gene_length)
...
How can we compare the results from the two MS experiments? They did not use identical identifiers...
Maybe we can merge by TopProteinName?
```{r}
intersect(ms2$TopProteinName, ms$TopProteinName) %>% length
intersect(ms2$Fasta.headers, ms$Fasta.headers) %>% length
intersect(ms2$ProteinName, ms$ProteinName) %>% length
```
But e.g. FUS does not have the same TopProteinName in both tables, so they are obviously not using the same identifiers...

ms1 sp|P56959|FUS_MOUSE
ms2 tr|G3UXT7|G3UXT7_MOUSE    ProteinName: tr|G3UXT7|G3UXT7_MOUSE;tr|Q8CFQ9|Q8CFQ9_MOUSE;sp|P56959|FUS_MOUSE

other example:
ms2 sp|P97930|KTHY_MOUSE;tr|D3Z3R3|D3Z3R3_MOUSE   sp|P97930|KTHY_MOUSE
ms1 sp|P97930|KTHY_MOUSE                          sp|P97930|KTHY_MOUSE


ms2 tr|A0A1W2P796|A0A1W2P796_MOUSE   tr|A0A1W2P796|A0A1W2P796_MOUSE;tr|E9QLW0|E9QLW0_MOUSE;sp|Q14B80|KCNC2_MOUSE;tr|A0A1W2P7B9|A0A1W2P7B9_MOUSE
ms  sp|Q14B80|KCNC2_MOUSE



It seems that in the second experiment, they were not parsing the correct.

We could parse the Uniprot IDs from the ProteinName and then try to match them to the UniProt IDs of the first experiment.
For the FGCZ specific IDs, we use them instead: zz|Y-FGCZCont00441| --> Y-FGCZCont00441


```{r}
## parse UniprotID from ProteinName
a <- ms2$ProteinName %>% str_match(pattern = "sp\\|([:alnum:]{6})\\|")
ms2$UniprotID <- a[,2]

## how many proteins do not have Uniprot IDs?
is.na(ms2$UniprotID) %>% table
## we ignore them for now
```

How many of the proteins were also detected in the first experiments?
```{r}
intersect(ms2$UniprotID, ms$UniprotID) %>% length
intersect(ms2$UniprotID, ms$UniprotID) %>% length/nrow(ms2)
```
Most of the proteins were also detected in the first experiment.
...
# res <- select(up, keys = ms2$UniprotID,
#               columns = c("PROTEIN-NAMES",  "ENTRY-NAME"),
#               keytype = "UNIPROTKB")
...
ms2$protein_name <- res[,"ENTRY-NAME"]
ms2$gene_name <- res[,"GENES"]
```
The gene names are not unique, some UniProt IDs refer to more than one gene name.

TODO: use the Ensembl gene IDs to match the gene name to the UniProt IDs!
Otherwise we cannot label the FUS targets and label our genes of interest


We export the table to a file.

```{r}
write.table(ms2, file = here("mass_spec/MQ_2_grp_report_p3273_o6609_SNS_KI_vs_WT_uniprot.txt"), 
            quote = FALSE, row.names = FALSE, sep = "\t")
```



Volcano plot of the second MS experiment of the SNS samples (KI vs. WT).

```{r}
dat_ms2 <- ms2 %>% dplyr::select(P.Value, adj.P.Val, log2FC, gene_name, protein_name, UniprotID) %>% 
dplyr::rename(PValue = P.Value, adjPValue = adj.P.Val, logFC = log2FC) %>%
  mutate(protein_name = str_remove(protein_name, pattern = "_MOUSE"))
...
ggplotVolcano(dat_ms2, file_name, label_coord = 3, 
              x_range = x_range, y_range = y_range, 
              gene_labels = gene_labels, FUS_targets = fus_genes,
              color_targets = FALSE)

file_name <- here("mass_spec/volcano_MS_experiment2.png")
ggplotVolcano(dat_ms2, file_name, label_coord = 3, 
              x_range = x_range, y_range = y_range, 
              color_targets = FALSE)
```
...
```{r}
dat_ms1 <- dat %>% dplyr::select(P.Value, adj.P.Val, log2FC_MS, gene_name, UniprotName, UniprotID) %>% 
dplyr::rename(PValue = P.Value, adjPValue = adj.P.Val, logFC = log2FC_MS, 
              gene_name = gene_name, protein_name = UniprotName) %>%
  mutate(protein_name = str_remove(protein_name, pattern = "_MOUSE"))

dat_full <- dat_ms2 %>% dplyr::select(PValue, adjPValue, logFC, UniprotID) %>% 
  inner_join(dat_ms1, by = "UniprotID", suffix = c("_2", "_1")   ) 
...
##all proteins with log2FC > 0.5 (noise)
p <- ggplot(dat_full %>% dplyr::filter(abs(logFC_1) > 0.5 | abs(logFC_2) > 0.5 ), 
            aes(x = logFC_1, y = logFC_2)) +
    geom_point(alpha = 0.7) + 
  theme_bw() + 
  xlab("log2FC experiment 1") +  ylab("log2FC experiment 2") +
  theme(aspect.ratio = 1)
p
ggsave(filename = here("mass_spec/log2FC_experiment_1_2_min0.5.pdf"))


p <- ggplot(dat_full, aes(x = ratio)) +
  geom_density() + 
  theme_bw() + 
  # xlab("log2FC experiment 1") +  ylab("log2FC experiment 2") +
  theme(aspect.ratio = 1) + 
...
dat_full$FUS_target <- dat_full$gene_name %in% fus_genes
